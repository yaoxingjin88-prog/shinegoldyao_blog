    ...
    <div class="max-w-6xl mx-auto px-6">
      ...
      <div class="text-center max-w-2xl mx-auto mb-16">
        <h1 class="text-4xl md:text-5xl font-extrabold mb-6">
          {{ $t('articles.title') }} <span class="bg-gradient-to-r from-cyan-600 to-emerald-500 bg-clip-text text-transparent">{{ $t('articles.titleHighlight') }}</span>
        </h1>
      ...
      <div class="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
        ...
        <div class="relative w-full md:w-64">
          ...
          <input v-model="keyword" type="text" :placeholder="$t('articles.search')" class="w-full pl-10 pr-4 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-full placeholder-gray-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all shadow-sm" @keyup.enter="page = 1; loadData()" />
        </div>
      </div>
      ...
      <div v-else class="text-center py-20 text-gray-400">{{ $t('articles.empty') }}</div>

      <!-- 分页 -->
      <div v-if="total > pageSize" class="flex justify-center mt-12 gap-2">
        <button :disabled="page <= 1" class="px-5 py-2 rounded-full border border-gray-200 dark:border-gray-700 text-sm disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors" @click="page--; loadData()">{{ $t('articles.prev') }}</button>
        <span class="px-4 py-2 text-sm text-gray-500">{{ page }} / {{ Math.ceil(total / pageSize) }}</span>
        <button :disabled="page >= Math.ceil(total / pageSize)" class="px-5 py-2 rounded-full border border-gray-200 dark:border-gray-700 text-sm disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors" @click="page++; loadData()">{{ $t('articles.next') }}</button>
      </div>
    </div>
  </div>
</template>
...
watch(initData, (val) => {
  if (!val) return
  categories.value = val.cats || []
  articles.value = val.articleRes?.list || []
  total.value = val.articleRes?.total || 0
}, { immediate: true })
